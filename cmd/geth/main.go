...
func prepare(ctx *cli.Context) {
	...
	case ctx.IsSet(utils.EnulsFlag.Name):
		log.Info("Starting Geth on Enuls mainnet...")
	case ctx.IsSet(utils.EnulsTestFlag.Name):
		log.Info("Starting Geth on Enuls testnet...")
...
`)
	...
	if ctx.String(utils.SyncModeFlag.Name) != "light" && !ctx.IsSet(utils.CacheFlag.Name) && !ctx.IsSet(utils.NetworkIdFlag.Name) {
		...
		if !ctx.IsSet(utils.RopstenFlag.Name) &&
			...
			!ctx.IsSet(utils.EnulsFlag.Name) &&
			!ctx.IsSet(utils.EnulsTestFlag.Name) &&

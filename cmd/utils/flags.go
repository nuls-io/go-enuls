...
var (
	...
	EnulsFlag = &cli.BoolFlag{
		Name:     "enuls",
		Usage:    "Enuls network",
		Category: flags.EthCategory,
	}
	EnulsTestFlag = &cli.BoolFlag{
		Name:     "enulstest",
		Usage:    "Enuls test network",
		Category: flags.EthCategory,
	}
	// Dev mode
...
func MakeDataDir(ctx *cli.Context) string {
	if path := ctx.String(DataDirFlag.Name); path != "" {
		...
		if ctx.Bool(EnulsFlag.Name) {
			return filepath.Join(path, "enuls")
		}
		if ctx.Bool(EnulsTestFlag.Name) {
			return filepath.Join(path, "enulstest")
		}
		return path
	}
...
func setBootstrapNodes(ctx *cli.Context, cfg *p2p.Config) {
	...
	case ctx.Bool(EnulsFlag.Name):
		urls = params.EnulsMainnetBootnodes
	case ctx.Bool(EnulsTestFlag.Name):
		urls = params.EnulsTestBootnodes
	}
...
func SetDataDir(ctx *cli.Context, cfg *node.Config) {
	...
	case ctx.Bool(EnulsTestFlag.Name) && cfg.DataDir == node.DefaultDataDir():
		cfg.DataDir = filepath.Join(node.DefaultDataDir(), "enulstest")
	case ctx.Bool(EnulsFlag.Name) && cfg.DataDir == node.DefaultDataDir():
		cfg.DataDir = filepath.Join(node.DefaultDataDir(), "enuls")
	}
}
...
func SetEthConfig(ctx *cli.Context, stack *node.Node, cfg *ethconfig.Config) {
	...
	case ctx.Bool(EnulsFlag.Name):
		log.Warn("")
		log.Warn("--------------------------------------------------------------------------------")
		log.Warn("Please note, Enuls has been deprecated. ")
		log.Warn("--------------------------------------------------------------------------------")
		log.Warn("")
		...
		cfg.Genesis = core.DefaultEnulsGenesisBlock()
		SetDNSDiscoveryDefaults(cfg, params.EnulsGenesisHash)
	case ctx.Bool(EnulsTestFlag.Name):
		log.Warn("")
		log.Warn("--------------------------------------------------------------------------------")
		log.Warn("Please note, Enuls Testnet has been deprecated. ")
		log.Warn("--------------------------------------------------------------------------------")
		log.Warn("")
		...
		cfg.Genesis = core.DefaultEnulsTestGenesisBlock()
		SetDNSDiscoveryDefaults(cfg, params.EnulsTestGenesisHash)
	case ctx.Bool(GoerliFlag.Name):
		if !ctx.IsSet(NetworkIdFlag.Name) {
			cfg.NetworkId = 5
		}
...
func MakeGenesis(ctx *cli.Context) *core.Genesis {
	...
	case ctx.Bool(EnulsFlag.Name):
		genesis = core.DefaultEnulsGenesisBlock()
	case ctx.Bool(EnulsTestFlag.Name):
		genesis = core.DefaultEnulsTestGenesisBlock()
	case ctx.Bool(DeveloperFlag.Name):

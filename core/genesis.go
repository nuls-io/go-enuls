...
func CommitGenesisState(db ethdb.Database, hash common.Hash) error {
	...
	if len(blob) != 0 {
		...
		case params.EnulsGenesisHash:
			genesis = DefaultEnulsGenesisBlock()
		case params.EnulsTestGenesisHash:
			genesis = DefaultEnulsTestGenesisBlock()
...
func (g *Genesis) configOrDefault(ghash common.Hash) *params.ChainConfig {
	...
	case ghash == params.EnulsGenesisHash:
		return params.EnulsChainConfig
	case ghash == params.EnulsTestGenesisHash:
		return params.EnulsTestChainConfig
...
func DefaultGenesisBlock() *Genesis {
	return DefaultEnulsGenesisBlock()
	//return &Genesis{
...
func DefaultEnulsGenesisBlock() *Genesis {
	g := new(Genesis)
	reader := strings.NewReader(EnulsAllocData)
	if err := json.NewDecoder(reader).Decode(g); err != nil {
		panic(err)
	}
	return g
}
func DefaultEnulsTestGenesisBlock() *Genesis {
	g := new(Genesis)
	reader := strings.NewReader(EnulsTestAllocData)
	if err := json.NewDecoder(reader).Decode(g); err != nil {
		panic(err)
	}
	return g

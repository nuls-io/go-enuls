...
func NewNode(datadir string, config *NodeConfig) (stack *Node, _ error) {
	...
	if config.EthereumGenesis != "" {
		...
		if config.EthereumGenesis == EnulsGenesis() {
			genesis.Config = params.EnulsChainConfig
			if config.EthereumNetworkID == 1 {
				config.EthereumNetworkID = 119
			}
		}

		if config.EthereumGenesis == EnulsTestGenesis() {
			genesis.Config = params.EnulsTestChainConfig
			if config.EthereumNetworkID == 1 {
				config.EthereumNetworkID = 120
			}
		}

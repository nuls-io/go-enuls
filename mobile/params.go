...
func EnulsGenesis() string {
	enc, err := json.Marshal(core.DefaultEnulsGenesisBlock())
	if err != nil {
		panic(err)
	}
	return string(enc)
}
func EnulsTestGenesis() string {
	enc, err := json.Marshal(core.DefaultEnulsTestGenesisBlock())
	if err != nil {
		panic(err)
	}
	return string(enc)
